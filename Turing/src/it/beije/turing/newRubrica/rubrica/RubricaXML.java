...
public class RubricaXML implements Rubrica {
	...
	public void caricaFileContatti(Scanner s) {
		System.out.println("Inserire il nome del file .xml a cui accedere(senza estensione):");
		String tmp = s.nextLine();
		if(!tmp.isBlank() && !tmp.isEmpty()) {
			if(new File(Paths.get("File",tmp.concat(".xml")).toString()).exists()) {
				setPath(Paths.get("File",tmp.concat(".xml")));
				setAllContact(manager.loadRubricaFromXML(path.toAbsolutePath().toString()));
	...
	public void modificaContatto(Scanner s) {
		...
		System.out.println(c);
		manager.writeRubricaXML(allContact, path.toAbsolutePath().toString());
