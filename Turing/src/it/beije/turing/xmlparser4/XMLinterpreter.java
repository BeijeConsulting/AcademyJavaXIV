	...
	public Node ParseRoot() throws Exception //Ritorna il nodo Root con tutti i figli e relativi attributi
	{
		...
		for(int i=0;i<list.size();i++)//per ogni riga 
		{
			...
			else if(s.startsWith("<")) //se incomincia con un angolare
			{
				...
				{
					name=s.substring(1,s.length()-1); //il nome del tag � la stringa senza angolari
				}
				if(!list.get(i+1).startsWith("<")) //se la riga dopo non � un tag
				{
					hasContent=true; //si tratta di testo
					StringBuilder builder = new StringBuilder();
					int k=i+1; //k � l'indica della riga seguente
					while(!list.get(k+1).startsWith("<")) //finch� la riga dopo � testo
					{
						builder.append(list.get(k));	//aggiungi al testo
						k++;
					}
					String endContent = list.get(k);  //questa � l'ultima riga del testo
					i=k;							// avanzo il ciclo oltre il testo
					k=endContent.indexOf('<');		//la chiusura del tag rimane, nel formato, attaccata al testo, cos� dove finisce il testo ed inizia il tag
					if(k==-1)	
					{
						throw new Exception("Errore nella formattazione di input oppure mancata chiusura di un nodo con testo");		//se non viene chiuso il tag c'� un errore
					}
					else
					{
					...
					if(endContent.substring(k+2,endContent.length()-1).equals(name))  //se il tag di chiusura corrisponde all'ultimo nodo aperto	
					{
						...
						if(stackCursor!=-1) //se non � il root
						{
							stack.get(stackCursor).addChild(node); //� il figlio di qualcuno
						}
						else
						{
							root=node;
						}
						//stack.add(node);//lo metto in stack anceh lui ma � gi� stato chiuso perci� non serve modificare il contatore
					}
					else 
					{
